...
FINAL_MDL=$(resolve_model_asset "$RUNTIME_ROOT" "final_mdl" "$MODEL_DIR/final.mdl")
TREE=$(resolve_model_asset "$RUNTIME_ROOT" "tree" "$MODEL_DIR/tree")
...
while IFS=$'\t' read -r graph_name grammar_fst; do
  ...
  if [[ "$has_model" == "true" ]]; then
    ...
    (
      ...
      cp "$FINAL_MDL" "$tmp_model_dir/final.mdl"
      cp "$TREE" "$tmp_model_dir/tree"
      if [[ -f "$MODEL_DIR/phones.txt" ]]; then
        cp "$MODEL_DIR/phones.txt" "$tmp_model_dir/phones.txt"
      fi
      utils/mkgraph.sh "$lang_graph_dir" "$tmp_model_dir" "$graph_dir"
      rm -rf "$tmp_model_dir"
    )
    note="graph_built"
    graph_out="$graph_dir"
...
def strip_build_status(notes: str) -> str:
    parts = []
    for raw_part in notes.split(";"):
        part = raw_part.strip()
        if not part or part.startswith("build_status="):
            continue
        parts.append(part)
    return "; ".join(parts)
...
for entry in manifest["entries"]:
    ...
    base_notes = strip_build_status(entry.get("notes", "").strip())
    entry["notes"] = base_notes
    entry["graph_status"] = note
