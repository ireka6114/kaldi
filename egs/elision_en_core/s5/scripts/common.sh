...
DEFAULT_DICT_NAME="dict_ELISION_EN"
DEFAULT_LANG_NAME="lang_elision_en"
DEFAULT_MODEL_DIR="$KALDI_ROOT/egs/t04_en_constrained/s5/runtime/model/m13_librispeech_resolved"
...
resolve_model_dir() {
  ...
  if [[ -f "$env_file" ]]; then
    # shellcheck source=/dev/null
    . "$env_file"
    if [[ -n "${ELISION_EN_MODEL_DIR:-}" ]]; then
      echo "$ELISION_EN_MODEL_DIR"
      return 0
    fi
  fi
